...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a TGIS address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no TGIS: URI
...
{
    // Convert tgis:// to tgis:
    //
    //    Cannot handle this later, because tgis:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open tgis.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "TGIS.lnk";
}
...
{
    return GetAutostartDir() / "tgis.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a tgis.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=TGIS\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the tgis app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add tgis app to startup item list
