...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeTgisAmount"))
        settings.setValue("nAnonymizeTgisAmount", 1000);

    nAnonymizeTgisAmount = settings.value("nAnonymizeTgisAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeTgisAmount"))
        SoftSetArg("-anonymizetgisamount", settings.value("nAnonymizeTgisAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in tgis.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeTgisAmount:
            return QVariant(nAnonymizeTgisAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeTgisAmount:
            nAnonymizeTgisAmount = value.toInt();
            settings.setValue("nAnonymizeTgisAmount", nAnonymizeTgisAmount);
            emit anonymizeTgisAmountChanged(nAnonymizeTgisAmount);
            break;
        case CoinControlFeatures:
